...
Store.create(name: "Surrey", annual_revenue: 244000, womens_apparel: true, mens_apparel: true)
...
@mens_stores.each { |store| pp store.name }

@womens_stores = Store.where(["womens_apparel = ? and annual_revenue < ?", "true", "1000000"])

@womens_stores.each { |store| pp store.name }
